...
openConnection();
authorize();
$question_authors = get_question_authors_as_array();
?>
<!DOCTYPE html>
<html lang="en">
    <head>
    ...
    <body style="background-image: url('../img/bglogo2.jpg');">
        <div id="framecontent">
            <h2 class="cooltitle2" style="border-bottom-style:solid;">Manage Question Authors</h2><br />
            [<a class="anchor"href="../manage_admin/manage_admin.php">Manage Admin</a>] | [<a class="anchor" href="../pc_registrar/pc_registrar.php">Manage PC Registrars</a>] | [<a class="anchor" href="../author/author.php">Manage Question Authors</a>]
            <form class="style-frm" id="question-author-frm" >
                <br />
                <fieldset><legend>Add/Modify Question Author</legend>
                    <b>Staff P.No.: </b> <input type="text" name="pno" id="pno" /> <input type="submit" name="load" id="load" value="Generate" />
                    <div id="staff-detail">
                    </div>
                </fieldset><br />
                <fieldset id="author-list"><legend>Available Question Author(s)</legend>
                    <table class="style-tbl">
                        <tr><th>S/N</th><th>P.No.</th><th>FULL NAME</th><th>Department</th><th>ACTION</th></tr>
                        <?php
                        $c = 1;
                        foreach ($question_authors as $question_author) {
                            $pno = get_staff_pno($question_author);
                            ...
                            echo "<tr><td>" . $c++ . "</td><td>" . strtoupper($pno) . "</td><td>" . $fullname . "</td><td>" . ucwords(strtolower($biodata['departmentname'])) . "</td><td>[<a data-uid='$question_author' data-fname='$fullname' class='remove' href='javascript:void(0)'>Remove</a>]</td></tr>";
                        }
                        if ($c == 1)
                            echo"<tr><td colspan='6'>No question author available.</td></tr>";
                        ?>
                    </table>
                </fieldset>
            </form>
        ...
        <script type="text/javascript">
            ...
            $(document).on('click','#submit',function(event){ 
                ...
                $.ajax({
                    type:'POST',
                    url:'register_author.php',
                    data:$("#question-author-frm").serialize()
            ...
            $(document).on('click','.remove', function(event){
                ...
                var dis=$(this);
                // alert(pno);
                if(window.confirm("Are you sure you want to remove '"+$(this).attr("#data-fname")+"' from the list of question authors?"))
                {
                    $.ajax({
                        url:'remove_question_author.php',
                        type:'POST',

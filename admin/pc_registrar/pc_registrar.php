...
openConnection();
authorize();
$pc_registrars = get_pc_registrars_as_array();
?>
<!DOCTYPE html>
<html lang="en">
    <head>
    ...
    <body style="background-image: url('../img/bglogo2.jpg');">
        <div id="framecontent">
            <h2 class="cooltitle2" style="border-bottom-style:solid;">Manage PC Registrar</h2><br />
            [<a class="anchor"href="../manage_admin/manage_admin.php">Manage Admin</a>] | [<a class="anchor" href="../test_admin/test_admin.php">Test Administrators</a>]
            <form class="style-frm" id="test-admin-frm" >
                <br />
                <fieldset><legend>Add PC Registrar</legend>
                    <b>Staff P.No.: </b> <input type="text" name="pno" id="pno" /> <input type="submit" name="load" id="load" value="Generate" />
                    <div id="staff-detail">
                    </div>
                </fieldset><br />
                <fieldset id="pc-registrar-list"><legend>Available pc registrar(s)</legend>
                    <table class="style-tbl">
                        <tr><th>S/N</th><th>P.No.</th><th>FULL NAME</th><th>Department</th><th>ACTION</th></tr>
                        <?php
                        $c = 1;
                        foreach ($pc_registrars as $pc_registrar) {
                            $pno = get_staff_pno($pc_registrar);
                            ...
                            echo "<tr><td>" . $c++ . "</td><td>" . strtoupper($pno) . "</td><td>" . $fullname . "</td><td>" . ucwords(strtolower($biodata['departmentname'])) . "</td><td>[<a data-uid='$pc_registrar' data-fname='$fullname' class='remove' href='javascript:void(0)'>Remove</a>]</td></tr>";
                        }
                        if ($c == 1)
                            echo"<tr><td colspan='6'>No PC Registrar available.</td></tr>";
                        ?>
                    </table>
                </fieldset>
            </form>
        ...
        <script type="text/javascript">
            ...
            $(document).on('click','.remove',function(event){ 
                ...
                if(window.confirm("Are you sure you want to remove \""+fname+"\" from existing pc registrar list."))
                {
                    $.ajax({
                        url:'remove_pc_registrar.php',
                        type:'POST',

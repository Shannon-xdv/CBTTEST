...
?></select></td></tr>
                ...
                <tr><td>Test Code:</td><td><select name="init-testcode"><option value="">--Select--</option><?php
                            $query = "select * from tbltestcode where testname <>'PUTME' && testname <>'SBRS' && testname<>'SBRS-NEW'";
...
?></select></td></tr>
                <tr><td>Test Type:</td><td><select name="init-testtype"><option value="">--Select--</option><?php
                            $query2 = "select * from tbltesttype";
...
?></select></td></tr>
            <tr><td>
                    <?php
                    $query3 = "select testcodeid from tbltestcode where testname=? limit 1";

					$stmt3 = $dbh->prepare($query3);
					$stmt3->execute(array('SBRS-NEW'));
					$row3=$stmt3->fetch(PDO::FETCH_ASSOC);
					$numrow= $stmt3->rowCount();
    ...
    <div class="scheme-select">
        ...
        <table id="test-init-tbl">
            ...
            <tr><td>Semester:</td><td><select name="init-semester"><option value="1">First</option><option value="2">Second</option><option value="3">Third</option></select></td></tr>
            <tr><td>Test Type:</td><td><select name="init-testtype"><option value="">--Select--</option><?php
                        $sql4 = "select * from tbltesttype";
						$stmt4 = $dbh->prepare($query4);
					    $stmt4->execute(array('SBRS-NEW'));

                        while ($row4=$stmt4->fetch(PDO::FETCH_ASSOC)) {
                            echo "<option value='" . $row4['testtypeid'] . "'>" . $row4['testtypename'] . "</option>";
                        }
                    ?></select></td></tr>
            <tr><td>

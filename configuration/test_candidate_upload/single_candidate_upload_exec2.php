    ...
    <body style="background-image: url('../img/bglogo2.jpg');">
        ...
        foreach ($regs as $reg) {
           ...
           // $test_subjects_str = trim(implode(",", $test_subjects), " ,");
            $test_subjects_str="";
            foreach($test_subjects as $tsub){
                $test_subjects_str=$test_subjects_str.$tsub['subjectid'].",";
            }
            $test_subjects_str=rtrim($test_subjects_str,",");
            ...
            if (count($schedules) == 0) {
                ...
                foreach ($test_subjects as $tsbj) {
                    $sub=$tsbj['subjectid'];
                    $query = "insert into tblcandidatestudent (scheduleid, candidateid, subjectid)values ('$schedule','$candidateid','$sub')";

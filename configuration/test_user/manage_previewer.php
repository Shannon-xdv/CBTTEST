...
$question_previewers = get_question_previewer_as_array($testid);
...
<html lang="en">
    ...
    <body style="background-image: url('../img/bglogo2.jpg');">
        <div id="framecontent">
            <h2 class="cooltitle2" style="border-bottom-style:solid;">Question Previewers</h2><br />
...
foreach ($question_previewers as $question_previewer) {
    ...
    $subject_preview_str = "";
    foreach ($subject_previewed as $sbjprev) {
        $sbjnm = get_subject_code($sbjprev);
        $subject_preview_str.=", " . strtoupper($sbjnm);
    }
   
    $subject_preview_str = trim($subject_preview_str, " ,");
    echo "<tr><td>" . $c++ . "</td><td>" . strtoupper($pno) . "</td><td>" . $fullname . "</td><td>" . ucwords(strtolower($biodata['departmentname'])) . "</td><td>$subject_preview_str</td><td>[<a href='modify_previewer.php?tid=" . $testid . "&pno=$pno'>Modify</a>][<a data-tid='$testid' data-uid='$question_previewer' data-fname='$fullname' class='remove_previewer' href='javascript:void(0)'>Remove</a>]</td></tr>";
}
if ($c == 1)
    echo"<tr><td colspan='6'>No test previewer available.</td></tr>";
?>
                    </table>
                </fieldset>
            </form>
        ...
        <script type="text/javascript">
            ...
            $(document).on('click','.remove_previewer',function(event){ 

                var dis=$(this);
                var tid=dis.attr("data-tid");
                var uid=dis.attr("data-uid");
                var fname=dis.attr("data-fname");
                if(window.confirm("Are you sure you want to remove \""+fname+"\" from existing test previewers list."))
                {
                    $.ajax({
                        type:'POST',
                        url:'remove_previewer.php',
                        ...
                        if(msg==1)//success
                        {
                            alert("User was successfully remove from list of previewer(s)!");
                            window.location.reload(true);
                        }
                        if(msg==2)// access denied
                        {
